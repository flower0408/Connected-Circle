...
import rs.ac.uns.ftn.svtkvtproject.exceptionhandling.exception.MalformedQueryException;
...
public class SearchServiceImpl implements SearchService {
    ...
    @Override
    public Page<PostDocument> simpleSearch(List<String> keywords, Pageable pageable) {
    ...
    @Override
    public Page<PostDocument> advancedSearch(List<String> expression, Pageable pageable) {
...
//                m -> m.field("title").fuzziness(Fuzziness.ONE.asString()).query(token)));
    ...
    private Page<PostDocument> runQuery(NativeQuery searchQuery) {

        var searchHits = elasticsearchTemplate.search(searchQuery, PostDocument.class,
            IndexCoordinates.of("post_document"));

        var searchHitsPaged = SearchHitSupport.searchPageFor(searchHits, searchQuery.getPageable());

        return (Page<PostDocument>) SearchHitSupport.unwrapSearchHits(searchHitsPaged);
    }
}
